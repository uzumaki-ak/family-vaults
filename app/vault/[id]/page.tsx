...
// import { TimelineView } from "@/components/timeline-view"
// import { MediaUpload } from "@/components/media-upload"
// import { TimeCapsuleUpload } from "@/components/time-capsule-upload"
// import { LockedMemories } from "@/components/locked-memories"
// import { CreateNoteDialog } from "@/components/create-note-dialog"
// import { SearchFilter } from "@/components/search-filter"
// import { VaultSettingsDialog } from "@/components/vault-settings-dialog"
// import { useToast } from "@/hooks/use-toast"
// import { ArrowLeft, Users, Share2, Grid3X3, Clock, Copy, Trash2, Music, FileText, Lock } from "lucide-react"
...
//     // Filter based on active tab
//     if (activeTab === "gallery") {
//       // Show all media types (excluding locked ones)
//       items = vault.media
//         .filter((media) => {
//           if (viewMode === "trash") {
//             return media.deletedAt && !media.isLocked
//           } else {
//             return !media.deletedAt && media.approved && !media.isLocked
//           }
//         })
//         .map((media) => ({
//           ...media,
...
//         }))
//     } else if (activeTab === "audio") {
//       // Show only audio (excluding locked ones)
//       items = vault.media
//         .filter((media) => {
//           const isAudio = media.type === "AUDIO"
//           if (viewMode === "trash") {
//             return media.deletedAt && isAudio && !media.isLocked
//           } else {
//             return !media.deletedAt && media.approved && isAudio && !media.isLocked
//           }
//         })
//         .map((media) => ({
//           ...media,
...
//         }))
//     } else if (activeTab === "notes") {
//       // Show only notes (excluding locked ones, only for active view)
//       items =
//         viewMode === "active"
//           ? vault.notes
//               .filter((note) => (!note.isPrivate || note.author.id === user?.id) && !note.isLocked)
//               .map((note) => ({
//                 ...note,
//                 type: "note",
//               }))
//           : []
//     } else if (activeTab === "locked-memories") {
//       // Show only locked memories for the current user
//       const lockedMedia = vault.media
//         .filter((media) => media.isLocked && media.uploader.id === user?.id)
//         .map((media) => ({
//           ...media,
//           type: "media" as const,
//           author: media.uploader,
...
//       const lockedNotes = vault.notes
//         .filter((note) => note.isLocked && note.author.id === user?.id)
//         .map((note) => ({
//           ...note,
//           type: "note" as const,
//           author: note.author,
...
//       items = [...lockedMedia, ...lockedNotes]
//     } else if (activeTab === "timeline") {
//       // Show all items for timeline (excluding locked ones)
//       const mediaItems = vault.media
//         .filter((media) => {
//           if (viewMode === "trash") {
//             return media.deletedAt && !media.isLocked
...
//                 <CreateNoteDialog vaultId={vault.id} onNoteCreated={fetchVault} />
//                 <MediaUpload vaultId={vault.id} onUploadComplete={fetchVault} />
//                 <TimeCapsuleUpload vaultId={vault.id} onUploadComplete={fetchVault} />
//               </>
//             )}
//           </div>
//         </div>
...
//               Notes
//             </TabsTrigger>
//             <TabsTrigger value="locked-memories" className="flex items-center gap-2">
...
//           <TabsContent value="locked-memories" className="mt-6">
//             <LockedMemories memories={filteredItems} currentUserId={user.id} onMemoryUnlock={fetchVault} />
//           </TabsContent>
...
// Helper function to transform media items consistently
const transformMediaItem = (media: any) => ({
  ...media,
  type: "media" as const,
  author: media.uploader,
  mediaType: media.type,
  comments: media.comments || [],
  // Ensure all necessary properties are preserved
...
// Helper function to transform note items consistently
const transformNoteItem = (note: any) => ({
  ...note,
  type: "note" as const,
  author: note.author,
  // Ensure all necessary properties are preserved
...
export default function VaultPage() {
  ...
  const applyFilters = () => {
    ...
    // Helper function to filter media based on view mode
    const filterMedia = (mediaItems: any[], additionalFilter?: (item: any) => boolean) => {
      return mediaItems.filter((media) => {
        // Apply additional filter first if provided
        if (additionalFilter && !additionalFilter(media)) {
          return false
        }
    ...
    if (activeTab === "gallery") {
      // Show all media types (excluding locked ones)
      const filteredMedia = filterMedia(vault.media)
      items = filteredMedia.map(transformMediaItem)
    } else if (activeTab === "audio") {
      // Show only audio (excluding locked ones)
      const filteredMedia = filterMedia(vault.media, (media) => media.type === "AUDIO")
      items = filteredMedia.map(transformMediaItem)
    } else if (activeTab === "notes") {
      // Show only notes (excluding locked ones, only for active view)
      items = viewMode === "active"
        ? vault.notes
            .filter((note) => (!note.isPrivate || note.author.id === user?.id) && !note.isLocked)
            .map(transformNoteItem)
        : []
    } else if (activeTab === "locked-memories") {
      // Show only locked memories for the current user
      const lockedMedia = vault.media
        .filter((media) => media.isLocked && media.uploader.id === user?.id)
        .map(transformMediaItem)
      ...
      // Show all items for timeline (excluding locked ones)
      const filteredMedia = filterMedia(vault.media)
      const mediaItems = filteredMedia.map(transformMediaItem)

      const noteItems = viewMode === "active"
        ? vault.notes
            .filter((note) => (!note.isPrivate || note.author.id === user?.id) && !note.isLocked)
            .map(transformNoteItem)
        : []
  ...
  // Create timeline items with consistent structure
  const timelineItems = filteredItems.map((item) => {
    const baseItem = {
      id: item.id,
      type: item.type,
